...
export default function Navbar() {
  ...
  const consultingServices = [
    { href: '/services/consulting/business-strategy', label: 'Business Strategy & Planning' },
    { href: '/services/consulting/operations-management', label: 'Operations Management' },
    { href: '/services/consulting/financial-advisory', label: 'Financial Advisory' },
    { href: '/services/consulting/change-management', label: 'Change Management' },
    { href: '/services/consulting/risk-assessment', label: 'Risk Assessment & Mitigation' },
    { href: '/services/consulting/performance-analytics', label: 'Performance Analytics' },
    { href: '/services/consulting/market-analysis', label: 'Market Analysis' },
    { href: '/services/consulting/organizational-development', label: 'Organizational Development' }
  ];

  const marketingServices = [
    { href: '/services/marketing/brand-strategy', label: 'Brand Strategy & Positioning' },
    { href: '/services/marketing/content-creation', label: 'Content Creation & Production' },
    ...
    { href: '/services/marketing/website-funnels', label: 'Website + Funnel Systems' },
    { href: '/services/marketing/seo', label: 'SEO + Awareness Growth' },
    { href: '/services/marketing/lead-generation', label: 'Lead Generation & Revenue' },
    { href: '/services/marketing/ecommerce', label: 'eCommerce Marketplace Growth' },
  ...
  return (
    <header className="fixed top-0 w-full z-50">
      ...
      <div className="bg-black text-white text-sm">
        <div className="max-w-7xl mx-auto px-6 flex justify-between items-center h-9">
          <div className="flex items-center gap-6">
            <a href="tel:+18133018706" className="hover:text-[#D5AF34] transition-colors duration-200 flex items-center gap-1.5">
              <span>📞</span> (813) 263-6762
            </a>
            <a href="mailto:grow@iconicbrandgroup.com" className="hover:text-[#D5AF34] transition-colors duration-200 hidden sm:flex items-center gap-1.5">
              <span>✉️</span> grow@iconicbrandgroup.com
            </a>
          </div>
          <div className="flex items-center gap-6">
            <a href="https://iconicbrandgroup.com/support" className="hover:text-[#D5AF34] transition-colors duration-200 font-medium">
              Support
            </a>
            <Link href="/contact" className="hover:text-[#D5AF34] transition-colors duration-200 font-medium hidden sm:block">
              Contact
            </Link>
          </div>
        </div>
        ...
        <div className="flex justify-between items-center h-20">
          ...
          <div className="hidden lg:flex items-center gap-1">
            ...
            >
              <button className="relative px-5 py-2 text-gray-700 font-medium transition-all duration-300 hover:text-black group flex items-center gap-1">
                Services
                <HiChevronDown className={`w-4 h-4 transition-transform duration-300 ${servicesDropdownOpen ? 'rotate-180' : ''}`} />
              ...
              {/* Dropdown Menu */}
              <div className={`absolute top-full left-0 mt-2 w-72 bg-white/95 backdrop-blur-2xl rounded-2xl shadow-2xl border border-gray-200 overflow-visible transition-all duration-300 ${servicesDropdownOpen ? 'opacity-100 translate-y-0 visible' : 'opacity-0 -translate-y-2 invisible'
                }`}>
                ...
                >
                  ...
                  {/* Consulting Submenu */}
                  <div className={`absolute left-full top-0 ml-2 w-72 bg-white/95 backdrop-blur-2xl rounded-2xl shadow-2xl border border-gray-200 transition-all duration-300 ${consultingSubmenuOpen ? 'opacity-100 translate-x-0 visible' : 'opacity-0 -translate-x-2 invisible'}`}>
                ...
                >
                  ...
                  {/* Marketing Submenu */}
                  <div className={`absolute left-full top-0 ml-2 w-72 bg-white/95 backdrop-blur-2xl rounded-2xl shadow-2xl border border-gray-200 transition-all duration-300 ${marketingSubmenuOpen ? 'opacity-100 translate-x-0 visible' : 'opacity-0 -translate-x-2 invisible'}`}>
          ...
          >
            ...
            <div className="absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2">
              <span className={`block w-6 h-0.5 bg-gray-700 transition-all duration-300 ${mobileMenuOpen ? 'rotate-45 translate-y-1.5' : '-translate-y-1'}`}></span>
              <span className={`block w-6 h-0.5 bg-gray-700 transition-all duration-300 ${mobileMenuOpen ? 'opacity-0' : 'opacity-100'}`}></span>
              <span className={`block w-6 h-0.5 bg-gray-700 transition-all duration-300 ${mobileMenuOpen ? '-rotate-45 -translate-y-1.5' : 'translate-y-1'}`}></span>
            </div>
          </button>
        </div>
      ...
      {/* Mobile Menu */}
      <div className={`lg:hidden transition-all duration-500 ${mobileMenuOpen ? 'max-h-[80vh] opacity-100 overflow-y-auto' : 'max-h-0 opacity-0 overflow-hidden'
        }`}>
        <div className="bg-white/95 backdrop-blur-2xl border-t border-gray-200 px-6 py-8">
          ...
          <div className="mb-4">
            ...
            <div className="mb-2">
              ...
              >
                <span className="font-medium">Consulting Services</span>
                <HiChevronDown className={`w-4 h-4 transition-transform duration-300 ${mobileConsultingOpen ? 'rotate-180' : ''}`} />
              </button>
              <div className={`overflow-hidden transition-all duration-300 ${mobileConsultingOpen ? 'max-h-96' : 'max-h-0'}`}>
                <Link
                  href="/services/consulting"
            ...
            <div>
              ...
              >
                <span className="font-medium">Marketing Services</span>
                <HiChevronDown className={`w-4 h-4 transition-transform duration-300 ${mobileMarketingOpen ? 'rotate-180' : ''}`} />
              </button>
              <div className={`overflow-hidden transition-all duration-300 ${mobileMarketingOpen ? 'max-h-[500px]' : 'max-h-0'}`}>
                <Link
                  href="/services/marketing"
                  onClick={() => setMobileMenuOpen(false)}
